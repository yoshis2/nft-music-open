...
const CreatorsHeader = () => {
  ...
  return (
    <header className="flex h-20 w-full shrink-0 items-center px-4 md:px-6 bg-slate-50">
      ...
      {/* Logo and Site Name (Visible on Mobile when menu is closed) */}
      <Link
        href="/"
        className="flex items-center gap-2 font-semibold lg:hidden ml-2" // lg:hidden でデスクトップでは非表示, ml-2でボタンとの間隔調整
        prefetch={false}
      >
      ...
      {isMobileMenuOpen && (
        ...
        >
          {/* モバイルメニュー内のロゴとサイト名は削除またはコメントアウト */}
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4 px-6">
            スリーネクストが運営する NFT Musicの販売サイト
          </p>
          <nav>
            <div className="grid px-6">
              <Link href="/creators/users/create" className="moblink-primary" prefetch={false}>
                <PackageIcon className="h-5 w-5" />
                ユーザー登録
              </Link>
              <Link href="/creators/users/update" className="moblink-primary" prefetch={false}>
                <MailIcon className="h-5 w-5" />
                ユーザー編集
              </Link>
              <Link href="/creators/nft/listing" className="moblink-primary" prefetch={false}>
                <PackageIcon className="h-5 w-5" />
                NFT一覧
              </Link>
              <Link href="/creators/nft/mint" className="moblink-primary" prefetch={false}>
      ...
      <Link href="/" className="mr-6 hidden lg:flex items-center gap-2 font-semibold" prefetch={false}>
        <MountainIcon className="h-6 w-6" />
        スリーネクストNFTミュージック (クリエイター)
        <span className="sr-only">Acme Inc</span>
      </Link>
      {/* Desktop Menu */}
      <nav className="hidden lg:flex h-full space-x-4 ml-auto items-center">
        <div className="inline-flex h-full" role="group">
          <Link href="/creators/users/create" className="group weblink-primary" prefetch={false}>
            <PackageIcon className="h-5 w-5 mr-1" />
            ユーザー登録
          </Link>
          <Link href="/creators/users/update" className="group weblink-primary" prefetch={false}>
            <MailIcon className="h-5 w-5 mr-1" />
            ユーザー編集
          </Link>
          <Link href="/creators/nft/listing" className="group weblink-primary" prefetch={false}>
            <PackageIcon className="h-5 w-5 mr-1" />
            NFT一覧
          </Link>
          <Link href="/creators/nft/mint" className="group weblink-primary" prefetch={false}>
